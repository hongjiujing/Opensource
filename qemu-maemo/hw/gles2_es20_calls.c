...
int gles2_getMaxVertexAttrib(void);
...
//used by eglMakeCurrent when initializing vertex arrays.
int gles2_getMaxVertexAttrib(void)
{
    int ret;
    hgl.glGetIntegerv(GL_MAX_VERTEX_ATTRIBS, &ret);
    return ret;
}
...
static void gles2_glApplyVertexAttrib(gles2_Array *va)
{
    ...
    hgl.glVertexAttribPointer(va->indx, va->size, va->type,
        va->normalized, stride, (GLvoid*)((GLchar*)va->ptr - va->delta));
    GLenum error = hgl.glGetError();
    if (error != GL_NO_ERROR) {
        GLES2_PRINT("glVertexAttribPointer(%d, %d, 0x%x, 0, %d, %p - %p\n)"
                    " failed with 0x%x!\n", va->indx, va->size, va->type,
                    va->normalized, va->ptr, (void*)va->delta, error);
    }
}
...
static unsigned gles2_GetCount(TGLenum pname)
{
    ...
    switch(pname) {
        ...
        case GL_COMPRESSED_TEXTURE_FORMATS: count = 4; break;
        ...
        case GL_SHADER_BINARY_FORMATS: count = 4; break;
...
GLES2_CB(glDisableVertexAttribArray)
{
    ...
    if(index < ctx->narrays) {
        ctx->arrays[index].enabled = 0;
    }
    else {
        GLES2_PRINT("ERROR: index out of bounds! index(%d) >= max(%d)\n", index, ctx->narrays);
    }
...
GLES2_CB(glEnableVertexAttribArray)
{
    ...
    //But still call host so GL error gets set
...
GLES2_CB(glGetVertexAttribPointerv)
{
    ...
    if(index < ctx->narrays) {
        ...
        GLES2_PRINT("ERROR: index out of bounds! index(%d) >= max(%d)\n", index, ctx->narrays);
        //TODO: GL Error won't be set in this case!
    }
}
...
GLES2_CB(glVertexAttribPointer)
{
    ...
    if (indx < ctx->narrays) {
        ...
        if(buffer) {
            GLES2_PRINT("Array %d data from buffer %d with offset 0x%x (%d elements every %d bytes)\n",
            indx, buffer, tptr, size, stride);
            va->ptr = (void*)(intptr_t)va->tptr;
            va->delta = 0;
            //apply immediately
            if(!va->apply) {
                va->apply = gles2_glApplyVertexAttrib;
            }
            va->apply(va);
        } else {
...
GLES2_CB(glGetShaderInfoLog)
{
    ...
    char *infolog = malloc((bufsize > 0) ? bufsize : 1);
    if (infolog) {
        *infolog = 0;
    }
    GLsizei length = 0;
...
GLES2_CB(glShaderSource)
{
    ...
    char** string_fgl = NULL;
    GLint* length_fgl = NULL;
    if (count > 0) {
        string_fgl = malloc(sizeof(char*)*count);
        length_fgl = malloc(sizeof(GLint)*count);
    }
...
GLES2_CB(glGetAttachedShaders)
{
    ...
    GLuint *shaders = NULL;
    if (maxcount > 0) {
        shaders = malloc(maxcount * sizeof(GLuint));
    }
...
GLES2_CB(glGetProgramInfoLog)
{
    ...
    GLES2_BARRIER_RET;
    if (infologp && bufsize > 0) {
        gles2_transfer(s, infologp, length + 1, infolog, 1);
    }
    if (lengthp) {
        gles2_put_TGLsizei(s, lengthp, length);
    }
...
GLES2_CB(glUniform1fv)
{
    ...
    GLfloat *v = NULL;
    if (count > 0) {
        TGLsizei nvs = count*1;
        v = malloc(nvs*sizeof(*v));
        TGLsizei i;
...
GLES2_CB(glUniformMatrix2fv)
{
    ...
    GLfloat *value = NULL;
    if (count > 0) {
        TGLsizei nfloats = 2*2*count;
        value = malloc(nfloats*sizeof(TGLfloat));
        TGLsizei i;
...
GLES2_CB(glDeleteFramebuffers)
{
    ...
    if (n > 0) {
        framebuffers = malloc(sizeof(GLuint)*n);
        GLsizei i;
        for (i = 0; i < n; ++i) {
...
GLES2_CB(glDeleteRenderbuffers)
{
    ...
    if (n > 0) {
        renderbuffers = malloc(sizeof(GLuint)*n);
        GLsizei i;
        for (i = 0; i < n; ++i) {
