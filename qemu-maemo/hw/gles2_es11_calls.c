...
static void gles1_apply_glColorPointer(gles2_Array *va)
{
    hgl.glColorPointer(va->size, va->type, 0,
        (GLvoid*)((GLchar*)va->ptr - va->delta));
    GLenum error;

    if ((error = hgl.glGetError()) != GL_NO_ERROR) {
        GLES2_PRINT("glColorPointer(%d, 0x%x, %d, %p - %d\n)"
            " failed with 0x%x!\n", va->size, va->type,
            va->stride, va->ptr, va->delta, error);
    }
}

static void gles1_apply_glNormalPointer(gles2_Array *va)
{
    hgl.glNormalPointer(va->type, 0,
        (GLvoid*)((GLchar*)va->ptr - va->delta));
    GLenum error;

    if ((error = hgl.glGetError()) != GL_NO_ERROR) {
        GLES2_PRINT("glNormalPointer(0x%x, %d, %p - %d\n)"
            " failed with 0x%x!\n", va->type,
            va->stride, va->ptr, va->delta, error);
    }
}
...
static void gles1_apply_glTexCoordPointer(gles2_Array *va)
{
    hgl.glTexCoordPointer(va->size, va->type, 0,
        (GLvoid*)((GLchar*)va->ptr - va->delta));
    GLenum error;

    if ((error = hgl.glGetError()) != GL_NO_ERROR) {
        GLES2_PRINT("glTexCoordPointer(%d, 0x%x, %d, %p - %d\n)"
            " failed with 0x%x!\n", va->size, va->type,
            va->stride, va->ptr, va->delta, error);
    }
}
...
static void gles1_apply_glVertexPointer(gles2_Array *va)
{
    hgl.glVertexPointer(va->size, va->type, 0,
        (GLvoid*)((GLchar*)va->ptr - va->delta));
    GLenum error;

    if ((error = hgl.glGetError()) != GL_NO_ERROR) {
        GLES2_PRINT("glVertexPointer(%d, 0x%x, %d, %p - %d\n)"
            " failed with 0x%x!\n", va->size, va->type,
            va->stride, va->ptr, va->delta, error);
    }
}
...
static void gles1_apply_glPointSizePointer(gles2_Array *va)
{
    hgl.glPointSizePointerOES(va->type, 0,
        (GLvoid*)((GLchar*)va->ptr - va->delta));
    GLenum error;

    if ((error = hgl.glGetError()) != GL_NO_ERROR) {
        GLES2_PRINT("glPointSizePointerOES(0x%x, %d, %p - %p)"
...
GLES2_CB(glColorPointer)
{
    ...
    GLint buffer;
    hgl.glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &buffer);
    ...
    if(buffer) {
        GLES2_PRINT("Color array data from buffer %d with offset 0x%x (%d elements every %d bytes)\n",
        buffer, pointerp, size, stride);
        va->ptr = (void*)(intptr_t)va->tptr;
        va->delta = 0;
        //apply immediately
        va->apply(va);
    } else {
        GLES2_PRINT("Array glColorPointer at 0x%x (%d elements every %d bytes)\n",
        pointerp, size, stride);
    }
}
...
GLES2_CB(glNormalPointer)
{
    ...
    if(buffer) {
        GLES2_PRINT("Normal array data from buffer %d with offset 0x%x (3 elements every %d bytes)\n",
        buffer, pointerp, stride);
        va->ptr = (void*)(intptr_t)va->tptr;
        va->delta = 0;
        //apply immediately
        va->apply(va);
    } else {
        GLES2_PRINT("Array glNormalPointer at 0x%x (3 elements every %d bytes)\n",
        pointerp, stride);
    }
}
...
GLES2_CB(glTexCoordPointer)
{
    ...
    if(buffer) {
        GLES2_PRINT("TexCoord array data from buffer %d with offset 0x%x (%d elements every %d bytes)\n",
        buffer, pointerp, size, stride);
        va->ptr = (void*)(intptr_t)va->tptr;
        va->delta = 0;
        //apply immediately
        ...
        GLES2_PRINT("Array glTexCoordPointer at 0x%x (%d elements every %d bytes)\n",
        pointerp, size, stride);
    }
}
...
GLES2_CB(glVertexPointer)
{
    ...
    if(buffer) {
        GLES2_PRINT("Vertex array data from buffer %d with offset 0x%x (%d elements every %d bytes)\n",
        buffer, pointerp, size, stride);
        va->ptr = (void*)(intptr_t)va->tptr;
        va->delta = 0;
        //apply immediately
        ...
        GLES2_PRINT("Array glVertexPointer at 0x%x (%d elements every %d bytes)\n",
        pointerp, size, stride);
    }
}
...
GLES2_CB(glPointSizePointerOES)
{
    ...
    va->buffer = buffer;
    if(buffer) {
        GLES2_PRINT("PointSize array data from buffer %d with offset 0x%x (1 elements every %d bytes)\n",
        buffer, pointerp, stride);
        va->ptr = (void*)(intptr_t)va->tptr;
        va->delta = 0;
        //apply immediately
        ...
        GLES2_PRINT("Array glPointSizePointerOES at 0x%x (1 elements every %d bytes)\n",
        pointerp, stride);
    }
}
