                           ...
                           const char *arg)
{
    ...
    if (!strcmp(target, "x")) {
        ...
        axis = 2;
    } else if (!strcmp(target, "xyz")) {
        int xval = 0, yval = 0, zval = 0;
        if (sscanf(arg, "%d,%d,%d", &xval, &yval, &zval) != 3) {
            return -1;
        }
        TRACE_LIS302DL("axis values %d, %d, %d", xval, yval, zval);
        lis302dl_trigger(s, 0, xval);
        lis302dl_trigger(s, 1, yval);
        lis302dl_trigger(s, 2, zval);
        return 0;
    } else {
        return -1;
    }
    ...
    TRACE_LIS302DL("axis %d value %d", axis, value);
...
static I2CSlaveInfo lis302dl_info = {
    ...
    .qdev.props = (Property[]) {
        ...
        DEFINE_PROP_INT32("axis_max", LIS302DLState, axis_max, 58),
        DEFINE_PROP_END_OF_LIST(),
    },
    .init = lis302dl_init,
...
static void n900_skinrotate_callback(void *opaque, int portrait)
{
    struct n900_s *s = opaque;
    s->lis302dl->info->change(s->lis302dl, "xyz",
                              portrait ? "-58,0,0" : "0,-58,0");
}
#endif
...
#error MAX_SERIAL_PORTS must be at least 3!
    ...
    qdev_prop_set_int32(s->lis302dl, "x", 0);
    qdev_prop_set_int32(s->lis302dl, "y", -58);
...
#ifdef CONFIG_SKINNING
    qemu_skin_add_switchstate_callback(n900_switchstate_callback, s);
    qemu_skin_add_rotate_callback(n900_skinrotate_callback, s);
    // the skin has already rotated at this point if requested,
    // so we need to follow that manually here...
    if (graphic_rotate) {
        n900_skinrotate_callback(s, 1);
    }
#endif
