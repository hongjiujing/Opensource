...
int skin_button_handle_mouse(SkinKey* button, int mousebtn)
{
    ...
    if(button->type == toggle || button->type == rotate) {
...
int skin_button_handle_key(SkinKey* button, int keypressed)
{
    ...
    if(!keypressed && (button->type == toggle || button->type == rotate)) {
        switch(newstate) {
            case ESkinBtn_Idle:
            case ESkinBtn_Highlighted:
                if(!button->event.mouseover) newstate = ESkinBtn_Active;
...
int skin_button_checkswitch(SkinScreen* skin, SkinButton* button)
{
    //printf(">> skin_button_checkswitch\n");
    int hwstate = unknown;
    if (button->key.type == toggle) {
        ...
        if (hwstate == inactive) {
            ...
            if (button == skin->keyboard.button && skin->keyboard.visible) {
                skin->keyboard.visible = 0;
                                        ...
                                        void *opaque)
{
    SkinButtonStateCallback *cb = switchcb;
    if (!cb) {
        switchcb = (SkinButtonStateCallback*)qemu_mallocz(sizeof(*switchcb));
        cb = switchcb;
    }
    else {
        while (cb->next) cb = cb->next;
        cb->next = (SkinButtonStateCallback*)qemu_mallocz(sizeof(*(cb->next)));
        cb = cb->next;
    }
