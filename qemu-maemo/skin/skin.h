...
#define ZOOM_MIN_FACTOR 50
#define ZOOM_MAX_FACTOR 130
...
typedef struct SkinButton {     // SkinButton and SkinKey share information
    ...
    struct SkinKey key;
    char* tooltip;
    uint8_t rcthide;            // show button only if RCT isn't connected
...
typedef struct SkinScreen {
    ...
    int rctwidth;           // dimensions when RCT is on, i.e.
    int rctheight;          // some optional buttons disappear
    struct SkinBackgroundColor bgcolor;
