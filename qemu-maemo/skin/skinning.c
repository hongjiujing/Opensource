...
typedef struct SkinRotateCallback {
    skinrotate_callback *callback;
    void *opaque;
    // Next callback
    struct SkinRotateCallback *next;
} SkinRotateCallback;
...
static SkinRotateCallback *rotatecb = NULL;
...
void qemu_skin_add_rotate_callback(skinrotate_callback *callback,
                                   void *opaque)
{
    SkinRotateCallback *cb = rotatecb;
    if (!rotatecb) {
        rotatecb = qemu_mallocz(sizeof(*rotatecb));
        cb = rotatecb;
    } else {
        for (; cb->next; cb = cb->next) {
        }
        cb->next = qemu_mallocz(sizeof(*(cb->next)));
        cb = cb->next;
    }
    if (cb) {
...
static void skin_notify_rotate(void)
{
    SkinRotateCallback *cb = rotatecb;
    for (; cb; cb = cb->next) {
        if (cb->callback) {
            cb->callback(cb->opaque, (skin->rotate == on));
        }
    }
}
...
void skin_handle_rotate(int force)
{
    if (force || (skin->rotate != skin->rotate_req)) {
        ...
        if (skin->keyboard.button && skin->keyboard.visible) {
			...
			fprintf(stderr, "###Update KEYBOARD 1###\n");
            ...
            if (keyboard_timer) qemu_del_timer(keyboard_timer);
			fprintf(stderr, "###Update KEYBOARD 2 ###\n");
...
int skinning_init(const char *skin_file, const char *id, int portrait)
{
    ...
    if (skin) {
        if (skin->rotate == on) {
            // we call notify here but usually there's nobody listening
            // yet so initial rotation is handled separately
            skin_notify_rotate();
            if (skin->keyboard.image && skin->keyboard.keys &&
                skin->keyboard.visible) {
                skin_position_items(1);
            }
        }
    }
...
{
    fprintf(stderr, "skin_host_resize(), w=%d, h=%d\n", ds_get_width(skin->ds), ds_get_height(skin->ds));
    if (!is_graphic_console()) return;
    if (startup) {
...
{
    fprintf(stderr, "skin_free_displaysurface\n");
    if (surface == NULL)
        return;
...
{
    fprintf(stderr, "skin_resize_displaysurface, width=%d, height=%d\n", width, height);
...
void skin_handle_zooming(void)
{
    fprintf(stderr, "skin_handle_zooming, zoom_factor=%d%%\n", skin->zoom_factor);
...
static void skin_key_handler(void *opaque, int keycode)
{
    ...
    while (button) {
        if ((!skin->rct || !button->rcthide) &&
            ...
            if (button->key.type == rotate && released) {
                skin->rotate_req = (skin->rotate_req == on) ? off : on;
            }
            
            // Zoom buttons
            if (button->key.type == zoom_in && released) {
            ...
            if (button->key.type == zoom_out && released) {
                                   ...
                                   void *opaque)
{
    ...
    if (is_skin_available() && ds) {
        ...
        if (skin->keyboard.button &&
			...
			fprintf(stderr, "Qemu console resize ###1\n");
            original_qemu_console_resize(skin->ds,
                                         SKIN_KB_WIDTH(skin),
                                         SKIN_KB_HEIGHT(skin));
        } else {
			fprintf(stderr, "Qemu console resize ###2...\n");
            original_qemu_console_resize(skin->ds, SKIN_WIDTH(skin), SKIN_HEIGHT(skin));
        }
...
{
    fprintf(stderr, ">> skinning: qemu_console_resize, width=%d, height=%d\n", width, height);
    ...
    fprintf(stderr, "skinning: qemu_console_resize >>\n");
}
