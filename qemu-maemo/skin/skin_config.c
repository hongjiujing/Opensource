...
static int skin_load_images(SkinScreen* skin, SkinLayout* layout)
{
    ...
    if (layout->keyboard && layout->keyboard->image->file != NULL) {
        ...
        skin->keyboard.rctscreenwidth = (layout->keyboard->rctscreenwidth
                                         ? layout->keyboard->rctscreenwidth
                                         : layout->keyboard->screenwidth);
        skin->keyboard.rctscreenheight = (layout->keyboard->rctscreenheight
                                          ? layout->keyboard->rctscreenheight
                                          : layout->keyboard->screenheight);
...
int skin_activate_layout(SkinScreen* skin, int rotate)
{
    int result = -1;
    ...
    //printf("skin_config.c: skin_activate_layout, rotate=%d\n", rotate);
    SkinLayout* layout = skin->config->landscape;
    if (!skin->config->landscape) {
        layout = skin->config->portrait;
        // Only portrait possible
        skin->rotate_req = skin->rotate = on;
    }
    if (rotate == on) {
        if (skin->config->portrait) layout = skin->config->portrait;
        // Only landscape possible
        else skin->rotate_req = skin->rotate = off;
    }
    ...
    if (layout->rctwidth == 0) {
        layout->rctwidth = layout->width;
    }
    if (layout->rctheight == 0) {
        layout->rctheight = layout->height;
    }
    if (layout->emuscreen_width == 0 || layout->emuscreen_height == 0) {
        // No default value given, make it 800x480 then
        layout->emuscreen_width = 800;
        layout->emuscreen_height = 480;
    }
    skin->width = layout->width;
    skin->height = layout->height;
    skin->rctwidth = layout->rctwidth;
    skin->rctheight = layout->rctheight;
...
static void parser_start_hndl(void *data, const char *element, const char **attr)
{
    ...
    if (value_is(element, "skin")) {
        for (i = 0; attr[i]; i += 2) {
            ...
            if (value_is(attr[i], "rctwidth")) {
                layout->rctwidth = atoi(attr[i + 1]);
            }
            if (value_is(attr[i], "rctheight")) {
                layout->rctheight = atoi(attr[i + 1]);
            }
        }
    }
    ...
    if (value_is(element, "keyboard")) {
        ...
        for (i = 0; attr[i]; i += 2) {
            ...
            if (value_is(attr[i], "rctscreenwidth")) {
                layout->keyboard->rctscreenwidth = atoi(attr[i + 1]);
            }
            if (value_is(attr[i], "rctscreenheight")) {
                layout->keyboard->rctscreenheight = atoi(attr[i + 1]);
            }
            if (value_is(attr[i], "red")) {
    ...
    if (value_is(element, "button")) {
        ...
        for (i = 0; attr[i]; i += 2) {
            ...
            if (value_is(attr[i], "type")) {
                if (value_is(attr[i + 1], "toggle")) {
                    latestButton->type = toggle;
                    latestButton->defaultstate = undefined;
                }
                if (value_is(attr[i + 1], "rotate")) {
                    latestButton->type = rotate;
                }
                if (value_is(attr[i + 1], "zoom_in")) {
                    latestButton->type = zoom_in;
                }
                if (value_is(attr[i + 1], "zoom_out")) {
                    latestButton->type = zoom_out;
                }
            }
            ...
            if (value_is(attr[i], "rcthide")) {
                if (value_is(attr[i + 1], "yes")) {
                    latestButton->rcthide = 1;
                }
            }
        }
    }
...
static void skin_layout_free(SkinLayout *layout)
{
    if (!layout)
        return;

    // free background
    SkinImageConfig *sic = layout->background;
    while (sic) {
        SkinImageConfig *next = sic->next;
        qemu_free(sic->file);
        qemu_free(sic);
        sic = next;
    }
    ...
    SkinKeyConfig *skc = layout->keyboard->keys;
    while (skc) {
        SkinKeyConfig *next = skc->next;
        qemu_free(skc);
        skc = next;
    }
    qemu_free(layout->keyboard);

    // free buttons
    SkinButtonConfig *skb = layout->buttons;
    while (skb) {
        SkinButtonConfig *next = skb->next;
        qemu_free(skb->tooltip);
        qemu_free(skb->image.file);
        qemu_free(skb);
        skb = next;
    }
...
static void skin_screen_free(SkinScreen *skin)
{
    if (!skin)
        return;

    // free buttons
    SkinButton *sb = skin->buttons;
    while (sb) {
        SkinButton *next = sb->next;
        qemu_free(sb->tooltip);
        qemu_free(sb);
        sb = next;
    }
    ...
    SkinKey *key = skin->keyboard.keys;
    while (key) {
        SkinKey *next = key->next;
        qemu_free(key);
        key = next;
    }
    SkinButton *button = skin->keyboard.button;
    while (button) {
        SkinButton *next = button->next;
        SkinKey *key = button->key.next;
        while (key) {
            SkinKey *next = key->next;
            qemu_free(key);
            key = next;
        }
        qemu_free(button->tooltip);
        qemu_free(button->image.data);
        qemu_free(button);
        button = next;
    }

    if (skin->keyboard.image) {
        qemu_free(skin->keyboard.image->data);
    }
    qemu_free(skin->keyboard.image);
    qemu_free(skin->keyboard.keys);
    qemu_free(skin->keyboard.button);

    // free font
    if (skin->font) {
        if (skin->font->image) {
            qemu_free(skin->font->image->data);
        }
        qemu_free(skin->font->image);
    }
    qemu_free(skin->font);

    // free configs
    if (skin->config) {
        skin_layout_free(skin->config->landscape);
        skin_layout_free(skin->config->portrait);
    }
