...
static void enqueue_event(events_state *s, unsigned int type, unsigned int code, int value)
{
	...
	if (type == EV_KEY)
	{
		fprintf(stderr, "##Key event: type=%d code=%d value=%d\n", type, code, value);
	}
	else if(type == EV_ABS)
	{
		fprintf(stderr, "##Touchscreen event: type=%d code=%d value=%d\n", type, code, value);
	}
...
void events_dev_init(uint32_t base, qemu_irq irq)
{
	...
	fprintf(stdout, "abs_cnt = %d\n", s->abs_info_count * sizeof(s->abs_info[0]));
}
