...
skin_window_process_event( SkinWindow*  window, SDL_Event*  ev )
{
...
#if 0
        //fprintf(stdout, "down: x=%d y=%d fx=%d fy=%d fis=%d\n",
    ...
    case SDL_MOUSEBUTTONUP:
		...
		delt_x = mx - fling_x;
		delt_y = my - fling_y;
		if (abs(delt_x) > abs(delt_y) && abs(delt_x) > 20)
		{
			fprintf(stderr, "horizational move.\n");
		}
		else if (abs(delt_y) > 20)
		{
			fprintf(stderr, "vertical move.\n");
		}
    ...
    case SDL_MOUSEMOTION:
		//fprintf(stdout, "%s: ev->motion_xrel = %d, ev->motion_yrel = %d\n", __func__, ev->motion.xrel, ev->motion.yrel);
        if ( window->ball.tracking ) {
